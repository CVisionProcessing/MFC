...
def main():
    ...
    TestImgLoader = torch.utils.data.DataLoader(test_image_folder_davis(TestData, False),batch_size=1, shuffle=False, num_workers=1, drop_last=False)
    model = MFC(args)
...
def evaluate(dataloader, model, log, of_model):
    ...
    for video_index, (lab_images, rgb_images, annotations, meta) in tqdm(enumerate(dataloader)):
        PFs = AverageMeter()
        PJs = AverageMeter()
        ...
        if args.pad_divisible > 1:
            ...
            cur_b, cur_c, cur_h, cur_w = lab_images[0].shape
            ...
            if (pad_h + pad_w) != 0:
                ...
                lab_images = [pad(x) for x in lab_images]
                annotations = [pad(x) for x in annotations]
                rgb_images = [pad(x) for x in rgb_images] if args.optical_flow_warp else None
                padded_height += pad_h
                padded_width += pad_w
        ...
        for i in range(N - 1):
            ...
            img_mem = [lab_images[ind].cuda() for ind in ref_index]
            msk_mem = [outputs[ind].cuda() for ind in ref_index]
            img_query = lab_images[i + 1].cuda()
            msk_query = annotations[i + 1].cuda()
            optical_flows = []
            if args.optical_flow_warp:
                flow_img_mem = [rgb_images[ind].cuda() for ind in ref_index]
                flow_img_query = rgb_images[i + 1].cuda()
                for img, ind in zip(flow_img_mem, ref_index):
                    long_gap = 15
                    iter_num = 5 if (i + 1) - ind > long_gap else 2
                    with torch.no_grad():
                        _, flow_up = of_model(img, flow_img_query, iters=iter_num, test_mode=True, up_scale=2)
                    optical_flows.append(flow_up)
            with torch.no_grad():
                s_ = time.time()
                optical_flows = torch.cat(optical_flows, 0)
                _output = model(img_mem, msk_mem, img_query, optical_flows, None)
                _output = F.interpolate(_output, (height, width), mode='bilinear')

...
def train(dataloader, model, optimizer, scaler, log, writer, epoch):
    ...
    for batch_ind, (images_lab, images_rgb, images_flow, meta) in enumerate(dataloader):
        model.train()
        ...
        ckpt_name = 'ckpt/raft-sintel.pth'
        of_model.load_state_dict(torch.load(ckpt_name))
        of_model.cuda()
        of_model.eval()
        img_option = []
        for i in range(len(images_lab[0])):
            with torch.no_grad():
                _, img_opt = of_model(images_rgb[0][i].unsqueeze(0), images_rgb[1][i].unsqueeze(0), iters=2, test_mode=True, up_scale=2)    
            img_option.append(img_opt)
        img_opt = torch.cat(img_option, 0)
        results = model(img_mem, msk_mem, img_query, img_opt, None)
